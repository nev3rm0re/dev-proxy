...
export class OpenAPIRecorder {
  ...
  recordRequest(req: Request, res: ServerResponse | Response, startTime: number) {
    ...
    const responseBody = 'responseBody' in res ? res.responseBody : (res as any).responseBody;
    const responseHeaders = 'getHeaders' in res ? res.getHeaders() : res.headers;
    ...
    ((this.paths[path][method].responses[statusCode] as ExtendedResponseObject)['x-response-examples'] || []).push({
      timestamp: startTime,
      body: responseBody
    });
  ...
  private updateParameters(operation: ExtendedOperationObject, req: Request) {
    ...
    Object.entries(req.query).forEach(([name, value]) => {
      if (!operation.parameters?.some(p => 
        !('$ref' in p) && p.in === 'query' && p.name === name
      )) {
    ...
    Object.keys(pathParams).forEach(name => {
      if (!operation.parameters?.some(p => 
        !('$ref' in p) && p.in === 'path' && p.name === name
      )) {
        operation.parameters = operation.parameters || [];
        operation.parameters.push({
          name,
    ...
    Object.keys(req.headers).forEach(name => {
      if (!operation.parameters?.some(p => 
        !('$ref' in p) && p.in === 'header' && p.name === name
      )) {
        operation.parameters = operation.parameters || [];
        operation.parameters.push({
          name,
